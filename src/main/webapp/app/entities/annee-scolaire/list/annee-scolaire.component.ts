...
import { EntityArrayResponseType, AnneeScolaireService } from '../service/annee-scolaire.service';
...
export class AnneeScolaireComponent implements OnInit {
  ...
  trackId = (_index: number, item: IAnneeScolaire): number => this.anneeScolaireService.getAnneeScolaireIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.anneeScolaires = this.refineData(dataFromBody);
  }

  protected refineData(data: IAnneeScolaire[]): IAnneeScolaire[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IAnneeScolaire[] | null): IAnneeScolaire[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.anneeScolaireService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
